...
public class ServerThread extends Thread {
	...
	private void handleRequest(HTTPRequest request, HTTPResponse response) {
		...
		if (requestMethod.equals("GET") || requestMethod.equals("POST")
			...
			File file = new File(JLWSConfig.DocumentRoot
					+ request.getHeaders().getURI());
			...
			if (file.exists()) {
				if (file.isDirectory()) {
					...
					if (request.getHeaders().getURI()
							.charAt(request.getHeaders().getURI().length() - 1) != '/') {
						...
						for (int i = 0; i < JLWSConfig.DirectoryIndex.size(); i++) {
							...
							fileExt = Utilities
									.getExtension((String) JLWSConfig.DirectoryIndex
											.elementAt(i));

							if (fileExt
									.equals(JLWSConfig.ServletMappedExtension)) {
								// checking for a class
								try {

									String index = (String) request
											.getHeaders().getURI()
											+ JLWSConfig.DirectoryIndex
													.elementAt(i);
									lss = new ServletService(
											new AndroidServletServiceDriver());
				...
				if (fileToBeServed != null) {
					...
					response.setContentType(JLWSConfig.MimeType
							.getMimeTypeByExtension(fileExt));
			 ...
			 */
				...
				if (fileExt.equals(JLWSConfig.ServletMappedExtension)) {
					...
					Log.i("SERVLET", "Attempt to load servlet for "
							+ request.getHeaders().getURI());
					...
					String asset_path = "public"
							+ request.getHeaders().getURI();
					Log.i("HTTP", "Attempt to serve asset " + asset_path);

					boolean asset_exists = false;
					try {
						android.content.res.AssetFileDescriptor afd = ((Context) MainController.getInstance().getContext()).getResources().getAssets()
								.openFd(asset_path);
					...
					if (!asset_exists) {
						...
						try {
							InputStream file_input = ((Context) MainController.getInstance().getContext()).getResources().getAssets()
									.open(asset_path);
							file_input.close();
							asset_exists = true;
						} catch (Exception e) {
