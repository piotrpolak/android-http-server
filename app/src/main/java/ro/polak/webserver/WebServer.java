...
public class WebServer extends Thread {
	...
	public static final String SERVER_VERSION = "1.2.1";
	public static final String SERVER_DATE = "01.06.2012";
	public static final String SERVER_SMALL_SIGNATURE = SERVER_NAME + "/"
			+ SERVER_VERSION;
	public static final String SERVER_SIGNATURE = SERVER_NAME + "/"
	...
	public void run() {
		...
		while (this.listen) {
			try {
				...
				if (this.listen) {
					this.controller
							.println("ERROR: IO exception while accepting socket.");
				}
			}
		}
	...
	public boolean startServer() {
		...
		if (!(new File(JLWSConfig.DocumentRoot).isDirectory())) {
			this.controller
					.println("ERROR: DocumentRoot does not exist! PATH: "
		...
		if (JLWSConfig.MaxThreads < 1) {
			this.controller
					.println("ERROR: MaxThreads should be greater or equal to 1!");
			return false;
		}
		...
		try {
			...
			e.printStackTrace();
			this.controller
					.println("ERROR: Unable to start server: unable to listen on port "
							+ JLWSConfig.Listen);
			return false;
		}

		this.controller.println("Server started. Listening on port "
				+ JLWSConfig.Listen);
		this.start();
		return true;
	}
