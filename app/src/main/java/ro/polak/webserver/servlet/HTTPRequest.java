		...
		try {
			...
			try {
				if (headers.getMethod().toUpperCase().equals("POST")) {
					...
					try {
						postLength = new Integer(
								headers.getHeader("Content-Length")).intValue();
					} catch (Exception e) {
					}
					...
					if (postLength > 0) {
						...
						if (headers.getHeader("Content-Type").startsWith(
								"multipart/form-data")) {
							/* Getting the boundary */
							String boundary = headers.getHeader("Content-Type");
							boundary = boundary.substring(
									boundary.indexOf("boundary=") + 9,
									boundary.length());
							mrh = new MultipartRequestHandler(in, postLength,
									boundary);
